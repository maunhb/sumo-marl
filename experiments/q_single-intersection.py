...
if __name__ == '__main__':
    ...
    prs.add_argument("-a", dest="alpha", type=float, default=0.1, required=False,
                                                     help="Alpha learning rate.\n")
    prs.add_argument("-g", dest="gamma", type=float, default=0.99, required=False, 
                                                     help="Gamma discount rate.\n")
    prs.add_argument("-e", dest="epsilon", type=float, default=0.9, required=False, 
                                                                 help="Epsilon.\n")
    prs.add_argument("-me", dest="min_epsilon", type=float, default=0.005,
                                        required=False, help="Minimum epsilon.\n")
    prs.add_argument("-d", dest="decay", type=float, default=0.99, required=False, 
                                                           help="Epsilon decay.\n")
    ...
    prs.add_argument("-r", dest="reward", type=str, default='wait2',
    ...
    prs.add_argument("-summaryfile", dest="summaryfile", 
                       default='outputs/summaryqsingle.xml', type=str, 
                       required=False, help="Choose a summary file name (.xml).\n")
    args = prs.parse_args()
    experiment_time = str(datetime.now()).split('.')[0]
    out_csv = 'outputs/my-single-intersection/q_{}_alpha{}_gamma{}_eps{}_decay{}'\
    '_reward{}'.format(experiment_time, args.alpha, args.gamma, 
    ...
    for run in range(1, args.runs+1):
        ...
        collect_ts_data =  CollectP2(ts_ids=env.ts_ids, phases=env.phases, 
                                        filename='outputs/CollectingP/singleq') 
