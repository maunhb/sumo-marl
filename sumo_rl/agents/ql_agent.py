...
class QLAgent(Agent):
    ...
    def __init__(self, starting_state, state_space, action_space, alpha=0.5,
                       gamma=0.95, exploration_strategy=EpsilonGreedy()):
    ...
    def learn(self, new_state, reward, done=False):
        ...
        s1 = new_state
        a = self.action
        self.q_table[s][a] += self.alpha*(reward + 
                                          self.gamma*max(self.q_table[s1]) - 
                                          self.q_table[s][a])
        self.state = s1
